...
namespace EPApi.DataAccess
{
    public sealed class LabelAssignmentsRepository
    {
        ...
        {
            // Para empezar soportamos estos tipos; amplía aquí cuando tengas otras tablas validadas.
            //return targetType is "patient" or "test" or "attempt" or "attachment" or "session";
        ...
        {
            // Validación de pertenencia por org según tipo soportado
        ...
        public async Task AssignAsync(Guid orgId, int labelId, string targetType, Guid targetId, CancellationToken ct = default)
        {
            ...
            // Idempotente: inserta sólo si no existe
            const string sql = @"
IF NOT EXISTS (
...
WHERE org_id=@org AND label_id=@lid AND target_type=@typ AND target_id_guid=@tid;";
        ...
        public async Task<IReadOnlyList<LabelsRepository.LabelRow>> ListForTargetAsync(
...
JOIN dbo.labels l
  ON l.id = la.label_id AND l.org_id = la.org_id
