...
namespace EPApi.Services.Storage
{
    public sealed class LocalStorageService : IStorageService
    {
        ...
        public LocalStorageService(IConfiguration cfg, IOptions<StorageOptions> options, IFileStorage fileStorage)
        {
...
END";
            ...
            CancellationToken ct)
        {
            ...
            // Copiamos a memoria para conocer tamaño exacto y SHA-256
            ...
            try
            {
                ...
                {
                    using (var cs = new CryptoStream(ms, sha, CryptoStreamMode.Write, leaveOpen: true))
                    {
                ...
                // Identificadores y storage_key
                var fileId = Guid.NewGuid();
                var safeName = MakeSafeFileName(originalName);
                var normalizedKey = StoragePathHelper.GetPatientAttachmentPath(orgId, patientId, fileId);
                ...
                try
                {
                    // Re-check de used_bytes dentro de la transacción
                    long usedBytes = 0;
                    ...
                    // Persistimos el archivo vía IFileStorage
                    ms.Position = 0;
                    var storedKey = await _fileStorage.SaveAsync(normalizedKey, ms, ct);

                    // Metadata en patient_files
                    const string INS = @"
                ...
                N'Local', @key, @sha, @comment, @uid, SYSUTCDATETIME(), NULL);";
                    await using (var cmd = new SqlCommand(INS, cn, (SqlTransaction)tx))
                    {
                        cmd.Parameters.AddRange(new[]
                        {
                            ...
                            new SqlParameter("@key", SqlDbType.NVarChar, 400){ Value = storedKey },
                            new SqlParameter("@sha", SqlDbType.Char, 64)
                            {
                                Value = BitConverter
                                    .ToString(shaHash)
                    ...
                    // Incrementar used_bytes en org_storage (upsert)
                    const string UPD = @"
        ...
        END";
                ...
                catch
                {
                    ...
                    // Limpieza best-effort del archivo si algo falló después de guardarlo
                    try { await _fileStorage.DeleteAsync(normalizedKey, ct); } catch { /* ignore */ }
        ...
        //            }
        //        }
        public async Task<(Stream content, string contentType, string downloadName)> OpenReadAsync(
    Guid fileId,
    CancellationToken ct)
        {
            const string SQL = @"
SELECT storage_key, content_type, original_name
